...
CAddress GetLocalAddress(const CNetAddr *paddrPeer)
{
    ...
    if (GetLocal(addr, paddrPeer))
    {
        ...
        if (pwalletMain) {
            ...
            ret.dAdvertisedBalanceCoins = (int64_t)(fraction * pwalletMain->GetBalance());
            if (fDebug) printf("GetLocalAddress: %s advertisedbalance : %lu\n", ret.ToStringIP().c_str() ,ret.dAdvertisedBalanceCoins);
        }
    }
    return ret;
}
...
std::map<CAddress, int> ListAdvertisedBalances()
{
    std::map<CAddress, int> result;
    ...
    for (
        ...
        if (0 < address->dAdvertisedBalanceCoins) {
            result[*address] = address->dAdvertisedBalanceCoins;
        }
    }
